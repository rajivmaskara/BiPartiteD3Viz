!function()
	...
	bP.partData = function(data,p){
		...
		sData.keys=[
			d3.set(data.map(function(d){ return d.Dim1;})).values().sort(function(a,b){ return ( a<b? -1 : a>b ? 1 : 0);}),
			d3.set(data.map(function(d){ return d.Dim2;})).values().sort(function(a,b){ return ( a<b? -1 : a>b ? 1 : 0);})		
		];
		...
		//console.log("Reached Here Again 1",sData);
		//data.forEach(function(d){
		//console.log("Reached Herein Loop",d.Metric1);
		//});
		
		//console.log("Reached Here Again 2",data.Dim1);
		//console.log("Reached Here Again 2",data.map(function(d){ return d.Metric1;}));
		
		data.forEach(function(d){ 
		    
			sData.data[0][sData.keys[0].indexOf(d.Dim1)][sData.keys[1].indexOf(d.Dim2)]=d.Metric1;
			sData.data[1][sData.keys[1].indexOf(d.Dim2)][sData.keys[0].indexOf(d.Dim1)]=d.Metric1; 
		});
		//console.log("SData:", sData);
		return sData;
	}
	
	bP.partData1 = function(data,p){
		var sData={};
		...
		data.forEach(function(d){ 
			...
			sData.data[0][sData.keys[0].indexOf(d.Dim1)][sData.keys[1].indexOf(d.Dim2)]=d.Metric2;
			sData.data[1][sData.keys[1].indexOf(d.Dim2)][sData.keys[0].indexOf(d.Dim1)]=d.Metric2; 
		});
		//console.log("SData:", sData);
		return sData;
	}
	...
	function drawHeader(header, id){
		...
		[0,1].forEach(function(d){
			...
			h.append("text").text(header[d]).attr("x", (c1[d]-120))
				.attr("y", -5).style("fill","grey");
			
			h.append("text").text(header[2]).attr("x", (c2[d]-10))
				.attr("y", -5).style("fill","grey");
